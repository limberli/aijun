...
Когда всё поднимется — открой **<http://localhost:5173>** (интерфейс **AI JUN V2**,
`aijun-ui`). Стек запускается в режиме `VITE_PLAN=local` — в шапке бейдж
**«Local model»**, без лимитов и без каких-либо запросов оплаты (локальная модель
ничего не стоит).
...
### Порт уже занят (8080/8081/8082/5173/11434)
...
### UI не отвечает / ошибка генерации сразу после старта
- **Причина:** оркестратор ещё не поднялся, либо контейнер `aijun-ui` ещё ставит
  зависимости (`npm install` при первом запуске).
- **Решение:** дождись `host-orchestrator` (healthy) и готовности `aijun-ui`, затем
  обнови <http://localhost:5173>. Проверь `docker compose -f docker-compose-qwen.yml ps`.
